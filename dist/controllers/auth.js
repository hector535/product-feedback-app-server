...
import { hashValue, createJWT, tryCatch, createEmailToken, getEmailFromToken, pickRandomPhoto, } from "../utils/index.js";
import { ACCESS_TOKEN_NAME, COOKIE_OPTIONS } from "../constants/index.js";
import { HttpStatusCode } from "../types/index.js";
export const signup = tryCatch(async (req, res) => {
    const { body: { name, username, email, password }, } = await zParser(authSchema.signup, req);
...
export const signin = tryCatch(async (req, res) => {
    const { body: { email, password }, } = await zParser(authSchema.signin, req);
    const authService = getAuthServices();
...
export const forgot = tryCatch(async (req, res) => {
    const { body: { email }, } = await zParser(authSchema.forgot, req);
    const authService = getAuthServices();
...
export const reset = tryCatch(async (req, res) => {
    const { body: { token, password }, } = await zParser(authSchema.reset, req);
...
export const activate = tryCatch(async (req, res) => {
    const { body: { token }, } = await zParser(authSchema.activate, req);
    const { email } = await getEmailFromToken(token);
    const authService = getAuthServices();
