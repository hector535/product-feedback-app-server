...
export const getAll = tryCatch(async (req, res) => {
    ...
    const { query: { limit, offset, status }, } = await zParser(feedbackSchema.getAll, req);
...
export const getByIdForPreview = tryCatch(async (req, res) => {
    const { uid } = await validateJWT(req);
    const { params: { id }, } = await zParser(globalSchema.paramsId, req);
    const feedbackService = getFeedbackServices();
...
export const getComments = tryCatch(async (req, res) => {
    ...
    const { params: { id, commentId }, } = await zParser(feedbackSchema.getComments, req);
    const feedbackService = getFeedbackServices();
...
export const add = tryCatch(async (req, res) => {
    const { uid } = await validateJWT(req);
    const { body: { title, content, category }, } = await zParser(feedbackSchema.add, req);
    const feedbackService = getFeedbackServices();
...
export const edit = tryCatch(async (req, res) => {
    const { uid } = await validateJWT(req);
    const { params: { id }, body: { title, content, category, status }, } = await zParser(feedbackSchema.edit, req);
    const feedbackService = getFeedbackServices();
