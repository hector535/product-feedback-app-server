...
export const add = tryCatch(async (req, res) => {
    ...
    const { body: { content, feedback, replyingTo }, } = await zParser(commentSchema.add, req);
...
export const edit = tryCatch(async (req, res) => {
    const { uid } = await validateJWT(req);
    const { params: { id }, body: { content }, } = await zParser(commentSchema.edit, req);
    const commentService = getCommentServices();
...
export const remove = tryCatch(async (req, res) => {
    const { uid } = await validateJWT(req);
    const { params: { id }, } = await zParser(commentSchema.remove, req);
    const commentService = getCommentServices();
